...
import (
	...
	"github.com/polynetwork/eos_relayer/cmd"
...
func setupApp() *cli.App {
	app := cli.NewApp()
	app.Usage = "EOS relayer Service"
	app.Action = startServer
	app.Version = config.Version
	app.Copyright = "Copyright in 2022 The CMIT"
	app.Flags = []cli.Flag{
		cmd.LogLevelFlag,
		cmd.ConfigPathFlag,
		cmd.EOSStartFlag,
		cmd.EOSStartForceFlag,
		cmd.PolyStartFlag,
		cmd.LogDir,
	}
...
func startServer(ctx *cli.Context) {
	...
	eosstart := ctx.GlobalUint64(cmd.GetFlagName(cmd.EOSStartFlag))
	if eosstart > 0 {
		StartHeight = eosstart
	}

	StartForceHeight = 0
	eosstartforce := ctx.GlobalUint64(cmd.GetFlagName(cmd.EOSStartForceFlag))
	if eosstartforce > 0 {
		StartForceHeight = eosstartforce
	}
	...
	// create eos sdk
	eosClient := eos.New(servConfig.EOSConfig.RestURL)
...
func waitToExit() {
	...
	go func() {
		for sig := range sc {
			log.Infof("waitToExit - EOS relayer received exit signal:%v.", sig.String())
			close(exit)
			break
		}
	}()
...
func initPolyServer(servConfig *config.ServiceEOSConfig, polysdk *sdk.PolySdk, eosClient *eos.API, boltDB *db.BoltDB) {
	mgr, err := manager.NewPolyManagerEOS(servConfig, PolyStartHeight, polysdk, eosClient, boltDB)
	if err != nil {
...
func main() {
	log.Infof("main - EOS relayer starting...")
