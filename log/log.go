...
func FileOpenTestData(path string) (*os.File, error) {
	if fi, err := os.Stat(path); err == nil {
		if !fi.IsDir() {
			return nil, fmt.Errorf("open %s: not a directory", path)
		}
	...
	logfile, err := os.OpenFile(path+currenttime+"_TestData.log", os.O_RDWR|os.O_CREATE, 0666)
	if err != nil {
		return nil, err
	}
	return logfile, nil
...
func InitLogTestData(logLevel int, a ...interface{}) *Logger {
	...
	if len(a) == 0 {
		...
		for _, o := range a {
			switch o.(type) {
			case string:
				logFile, err = FileOpenTestData(o.(string))
				if err != nil {
	...
	return &Logger{
		level:   logLevel,
		logger:  log.New(fileAndStdoutWrite, "", log.Ldate|log.Lmicroseconds),
		logFile: logFile,
	}
}
